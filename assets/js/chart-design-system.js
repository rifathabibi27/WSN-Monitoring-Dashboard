...
const ChartDesignSystem = (() => {
  ...
  const ThresholdManager = (() => {
    ...
    return {
      add(threshold) {
        ...
        const index = thresholds.findIndex((item) => item.id === threshold.id);
        if (index >= 0) {
      ...
      has(id) {
        return thresholds.some((item) => item.id === id);
      },
      get(id) {
        return thresholds.find((item) => item.id === id) ?? null;
      },
      remove(id) {
  ...
  /* ==========================================================
       DATASET STYLE
    ========================================================== */
  function createDataset(label, color, metadata = {}) {
    return {
      label,
      data: [],
      borderColor: color,
      backgroundColor(context) {
        return ChartDesignSystem.createGradient(context, color);
      },
      fill: true,
      borderWidth: 2.5,
      tension: 0.35,
  ...
  /* ==========================================================
    THRESHOLD DATASET
    ========================================================== */
  function createThreshold({ id, label, value, style = {}, visible = true }) {
    return {
      id:
        id ??
  ...
  /* ==========================================================
    ...
    ========================================================== */
  function buildThresholdData(labels, value) {
    if (!Array.isArray(labels)) {
      return [];
    }
    return Array(labels.length).fill(Number(value));
  }
  /* ==========================================================
    THRESHOLD RENDERER
    ========================================================== */
  function renderThresholdDataset(threshold, labels) {
    return {
      label: threshold.label,
      data: buildThresholdData(labels, threshold.value),
      borderColor: threshold.style.color,
      borderWidth: threshold.style.width,
      borderDash: threshold.style.dash,
      pointRadius: 0,
      pointHoverRadius: 0,
      fill: false,
      tension: 0,
  ...
  /* ==========================================================
    ...
    ========================================================== */
  function renderThresholds(labels = [], collection = null) {
    const source = collection ?? ThresholdManager;
    return source
      .getAll()
      .filter((threshold) => threshold.visible)
      .map((threshold) => renderThresholdDataset(threshold, labels));
  }
  /* ==========================================================
    DATASET COMPOSER
    ========================================================== */
  function composeDatasets({
    ...
    if (includeThresholds) {
      output.push(...renderThresholds(labels, thresholdCollection));
    }
    return output;
  }
  /* ==========================================================
  ...
  function filterDatasets(datasets = []) {
    return datasets.filter((dataset) => !dataset.isThreshold);
  }
  /* ==========================================================
    REGISTER THRESHOLD
    ========================================================== */
  function registerThreshold({ id, label, value, style, visible = true }) {
  ...
  /* ==========================================================
    TOGGLE THRESHOLD
    ========================================================== */
  function toggleThreshold(id, visible) {
    const threshold = ThresholdManager.get(id);
    if (!threshold) {
      return;
    }
